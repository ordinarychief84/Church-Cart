...
const schema = z
  .object({
    ...
    JWT_SECRET: z
      .string()
      .min(32, "JWT_SECRET must be at least 32 chars (256 bits of entropy recommended)"),
...
// Note: production-only secret requirements (e.g. PAYSTACK_WEBHOOK_SECRET) are
// enforced at the call-site (`verifyPaystackSignature` rejects unsigned and
// unmatched payloads in any environment) rather than at module load, so a
// `next build` doesn't fail before deploy-time secrets are wired up.
